...
def combine_chat_histories(chat_histories):
    ...
    """
    return ChatHistory([message for chat_history in chat_histories for message in chat_history.messages])
...
class ChatFlow(BaseFlow):
    ...
    def __init__(self, messages: List[Message], default_chat_llm: Optional[ChatLLM] = None,
                 default_input_chat_history: Optional[ChatHistory] = None, verbose: bool = False) -> None:
        """
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
             input_chat_history: Optional[ChatHistory] = None) -> Tuple[Dict[str, str], Tuple[List[ChatHistory], List[ChatHistory]]]:
        """
        ...
        for i in range(len(self._messages)):
            ...
            new_input_chat_histories.append(ChatHistory(messages))
        ...
        return variables, ([new_input_chat_histories[0]], internal_chat_histories)

    def __call__(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
                 input_chat_history: Optional[ChatHistory] = None, return_all: bool = True) -> Tuple[Dict[str, str], ChatHistory]:
        """
        Runs the chat flow through an LLM.
        ...
        """
        return_variables, _ = self.flow(input_variables, chat_llm=chat_llm,
    ...
    def compress_histories(input_histories: List[ChatHistory], internal_histories: List[ChatHistory]) -> Tuple[ChatHistory, ChatHistory]:
        """
        Combines a list of chat histories into one chat history.
        ...
        return (
            input_histories[0],
            ChatHistory([message for chat_history in internal_histories for message in chat_history.messages])
        )
...
class FuncChatFlow(ChatFlow):
    ...
    def __init__(self, func: Callable[[dict, Optional[ChatLLM], Optional[ChatHistory]], Tuple[Dict[str, str], Tuple[List[ChatHistory], List[ChatHistory]]]],
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
        ...
        outputs, (new_input_chat_histories, internal_histories) = self._func(input_variables, chat_llm=chat_llm,
                                                                             input_chat_history=input_chat_history)
        ...
        if len(new_input_chat_histories) != 1:
            raise ValueError(f'FuncChatFlow func must return a single input chat history. Received {len(new_input_chat_histories)}')
...
class ConditonalChatFlow(ChatFlow):
    ...
    def __init__(self, decision_chat_flow: ChatFlow, branch_chat_flows: Dict[str, ChatFlow],
                 share_input_history: bool = True, share_internal_history: bool = True,
                 default_chat_llm: Optional[ChatLLM] = None,
                 default_input_chat_history: Optional[ChatHistory] = None, verbose: bool = False):
        """
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
        ...
        if input_chat_history is None:
            input_chat_history = self.default_input_chat_history
        decision_variables, decision_histories = \
            self._decision_chat_flow(input_variables, chat_llm=chat_llm, input_chat_history=input_chat_history)

        decision_histories = self._decision_chat_flow.compress_histories(decision_histories)
        ...
        branch_variables, branch_histories = \
            branch_chat_flow(input_variables, chat_llm=chat_llm, input_chat_history=input_chat_history)
        ...
        return branch_variables, ([decision_histories[0], branch_histories[0]],
                                  [decision_histories[1], branch_histories[1]])

    def compress_histories(histories: Tuple[List[ChatHistory], List[ChatHistory]]) -> Tuple[ChatHistory, ChatHistory]:
        """
        Combines a list of chat histories into one chat history.
...
class SequentialChatFlows(ChatFlow):
    ...
    def __init__(self, chat_flows: List[ChatFlow], default_chat_llm: Optional[ChatLLM] = None,
                 default_input_chat_history: Optional[ChatHistory] = None, verbose: bool = False) -> None:
        """
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
        ...
        for chat_flow in self._chat_flows:
            chat_flow_output_variables, chat_flow_histories = \
                chat_flow(input_variables, chat_llm=chat_llm, input_chat_history=full_chat_history)
            ...
            if first_input_history is None:
                ...
                full_chat_history = combine_chat_histories([full_chat_history, chat_flow_histories[1]])
...
class ConcurrentChatFlows(ChatFlow):
    def __init__(self, chat_flows: List[ChatFlow], default_chat_llm: Optional[ChatLLM] = None,
                 default_input_chat_history: Optional[ChatHistory] = None, num_threads=None,
                 verbose: bool = False) -> None:
        """
        ...
        # Check not needed as output variables are not used in the input variables to others
        #if len(self._input_varnames.intersection(self._output_varnames)) > 0:
        #    raise ValueError(f'Some output variable names are also in the input variable names.\nInput: {self._input_varnames}\nOutput: {self._output_varnames}')
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
             input_chat_history: Optional[ChatHistory] = None) -> Tuple[Dict[str, str], List[ChatHistory]]:
        """
        ...
        :param chat_llm: Optional chat language model to use for the chat flow.
        :param input_chat_history: Optional input chat history.
        :return: Tuple of dictionary of output variables and list of chat histories (order matches ordering of chat_flows).
        """
        if chat_llm is None:
            chat_llm = self.default_chat_llm
        if input_chat_history is None:
        ...
        def wrapper(chat_flow):
            ...
            variables, histories = chat_flow(input_variables, chat_llm=chat_llm,
                                             input_chat_history=input_chat_history)
...
class ChatSpiral(ChatFlow):
    ...
    def __init__(self, chat_flow: ChatFlow, input_variables: dict,
                 output_varnames_remap: Optional[Dict[str, str]] = None) -> None:
        ...
        # check output contains all input variables?
    ...
    def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
        ...
        :return: Tuple of dictionary of output variables and list of chat histories.
        """
        output_variables, chat_history = self._chat_flow(input_variables, chat_llm=chat_llm,
                                                         input_chat_history=input_chat_history)
    ...
    def spiral(self, chat_llm: ChatLLM) -> Tuple[Dict[str, str], ChatHistory]:
        # option: history reset each flow?
        # 
        ...
        try:
            while True:
                new_variables, new_internal_history = self.flow(variables, chat_llm=chat_llm,
...
class ChatFlowManager:
    ...
    class ChatFlowNode:
        ...
        def flow(self, input_variables: dict, chat_llm: Optional[ChatLLM] = None,
                 input_chat_histories: Dict[str, Dict[str, ChatHistory]] = None) -> Tuple[Dict[str, str], List[ChatHistory]]:
            """
            ...
            output_variables, chat_histories = self.chat_flow(input_variables, chat_llm=chat_llm,
                                                              input_chat_history=input_chat_history)
    ...
    def flow(self, input_variables: dict, chat_llms: Optional[Dict[str, ChatLLM]] = None,
             input_chat_histories: Optional[Dict[str, ChatHistory]] = None) -> Tuple[Dict[str, str], List[ChatHistory]]:
        """
        ...
        for chat_flow_node in self.chat_flows_nodes:
            variables, internal_chat_history = chat_flow_node.flow(input_variables, chat_llm=chat_llms.get(chat_flow_node.name, None),
